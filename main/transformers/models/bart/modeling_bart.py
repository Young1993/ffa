...
class BartAttention(nn.Module):
    ...
    def __init__(
        ...
        # @@@
        # self.talk_matrix = torch.nn.Parameter(torch.Tensor(torch.normal(0, 0.1/(1024 ** 0.5), (self.num_heads, self.num_heads))))
        # self.tmp = 0
    ...
    def forward(
        ...
        # self-attention 中 add_talk_attention
        # if self.tmp == 1:
        #    attn_tmp = attn_weights.view(bsz, -1, tgt_len, tgt_len)
        #    attn_tmp = torch.einsum('abcd,eb->abcd', attn_tmp, self.talk_matrix) + attn_tmp
        #    attn_weights = attn_tmp.contiguous().view(-1, tgt_len, tgt_len)
        # self.tmp = 0

        # cross_attention 中 add_talk_attention
