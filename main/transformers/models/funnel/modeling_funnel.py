...
class FunnelRelMultiheadAttention(nn.Module):
    def __init__(self, config, block_index):
        ...
        # @@@
        self.talk_matrix = torch.nn.Parameter(torch.Tensor(torch.normal(0, 0.1/(1024 ** 0.5), (config.n_head, config.n_head))))
    ...
    def forward(self, query, key, value, attention_inputs, output_attentions=False):
        ...
        # add talk_attention
        content_score = torch.einsum('abcd,eb->abcd', content_score, self.talk_matrix) + content_score
